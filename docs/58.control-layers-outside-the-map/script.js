    ...
    return L.marker(latlng, {
      ...
      .bindPopup(
        ...
        <div class="box box-down cyan">
          <h2>${feature.properties.amenity}</h2>
...
function generateButton(name) {
  ...
  $(".layers").append(`
    <li>
      <input type="checkbox" id="${id}" name="item" class="item" value="${name}"> ${name}
...
const arrayLayers = ["bar", "pharmacy", "restaurant","hotel","polygon"];
...
arrayLayers.map((json) => {
  ...
  fetchData(`./data/${json}.json`).then((data) => {
    ...
    console.log(window["layer_" + json]);
  });
...
function showHideLayer(target) {
  console.log('showHideLayer :::: ',target.id);
  if (target.id === "all-layers") {
    arrayLayers.map((json) => {
      layercheckedType(json, target.checked);
    });
  } else {
    layercheckedType(target.id, target.checked);
  }
...
function layercheckedType(id, type) {
  console.log(id, type);
